...
// ─────────────────────────────────────────
// BloodSugarMenuSelect.dart의 복사본
// 원본은 선택 시 BloodSugarCheckRecord로 push하지만,
// 이 버전은 선택 시 Navigator.pop(context, 선택된 메뉴)으로
// 이전 화면(Example.dart)에 데이터를 돌려줌
// ─────────────────────────────────────────
class BloodSugarMenuSelectForExample extends StatefulWidget {
  const BloodSugarMenuSelectForExample({super.key});

  @override
  State<BloodSugarMenuSelectForExample> createState() =>
      _BloodSugarMenuSelectForExampleState();
}

class _BloodSugarMenuSelectForExampleState
    extends State<BloodSugarMenuSelectForExample> {
  ...
  List<Map<String, dynamic>> get _filteredMenus {
    var filtered = _allMenus.where((m) {
      ...
      if (_searchText.isNotEmpty &&
        ...
        return false;
      if (_selectedBrands.isNotEmpty && !_selectedBrands.contains(brand))
        return false;
      return true;
    }).toList();
    ...
    if (_selectedSort == '당류 낮은순') {
      ...
      filtered.sort(
        (a, b) => ((a['calories'] as num?) ?? 0).compareTo(
          (b['calories'] as num?) ?? 0,
        ),
      );
    }
                  ...
                  child: Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        // 선택 버튼
                        // ─────────────────────────────────────────
                        // 원본과 다른 부분: push 대신 pop으로 선택된
                        // 메뉴 데이터를 Example.dart에 돌려줌
                        // ─────────────────────────────────────────
                        SafeArea(
                          top: false,
                          child: Padding(
                            ...
                            child: GestureDetector(
                              onTap: _selectedMenu != null
                                  ? () => Navigator.pop(context, _selectedMenu)
                                  : null,
                              child: Container(
                                width: double.infinity,
                                height: 52,
